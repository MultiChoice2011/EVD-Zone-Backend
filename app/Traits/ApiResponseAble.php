...
trait ApiResponseAble
{
     ...
     * @author <ferasbbm>
     */
    protected function ApiSuccessResponseAndToken($data = null, $message = null,$token): JsonResponse
    {
        $message = $message ?? trans('api.Ok');
        $response = $this->successResponseAndToken(Response::HTTP_OK, $data, $message,$token);
        return response()->json($response);
    }
     ...
     * @author <ferasbbm>
     */
    private function successResponseAndToken(int $code = Response::HTTP_OK, $data = [], string $message = null,$token): array
    {
